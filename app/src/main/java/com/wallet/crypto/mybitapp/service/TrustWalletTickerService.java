...
public class TrustWalletTickerService implements TickerService {
    ...
    @Override
    public Single<Ticker> fetchTickerPrice(String symbols) {
        return apiClient
                .fetchTickerPrice(symbols)
                .map(r -> {
                    TrustResponse response = r.body();
                    if (response != null && response.response.length > 0) {
                        return response.response[0];

                    } else {
                        return Ticker.getEmpty();
                    }
                })
    ...
    public interface ApiClient {
        ...
        Single<Response<TrustResponse>> fetchTickerPrice(@Query("symbols") String symbols);
    }
